...
これは，`perl` というコマンド(命令) を使って，`01_hello.pl` というPerl 言語で書かれたプログラムファイルを実行した．ということです．
...
実は，Mac や Linux の人には必要ありません．
このプログラムファイルは，`perl` で実行する．というのはこの行に書かれています．
...
慣れないうち，慣れていても，情報が多い方が何か起きたときに気付きやすいので，
この 2行は基本的には付けるようにしましょう．
...
`;` (セミコロン) が，1つの文の区切りに必要になります．

## 練習問題

* 一人でやってみよう
    * 他にも何かを表示してみよう(名前，年齢)
* `perldoc -u -f atan2` を実行してみよう
* 以下のソースコードをファイルに保存して，実行して，結果を比較してみよう．何が変わりましたか？
...
TODO リストプログラムを作ります．どんな機能があるでしょうか？
私達の作る TODOリストは，以下のような機能を持っています．
...
* 1つめは，ファイルハンドル，`my $fh` の部分です．ファイルハンドルというのは，ファイルを扱うためのスカラ値です．
...
`<>` は，`< + ファイルハンドル + >` のように真ん中に引数を書きます．

ですので，`<STDIN>`の引数は，`STDIN` です．
`STDIN` はファイルハンドルです．`open()` 関数のところで出てきたものと同じです．
...
Perl がうまくやってくれるので，こういうことが起きている．
ということだけ知っておいてください．
...
この会で Perl プログラミングについて学んだことをまとめます
