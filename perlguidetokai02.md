...
`use` することで，関数は自動的に `関数名()` で呼び出せるようになっていましたが，
このように自動では `関数名()` で呼び出せず，
`モジュール名::関数名()` の形式で呼び出すために，
...
読み易さのために，ブロックの中は，ブロックの外よりも *字下げ* を入れるようにします．
